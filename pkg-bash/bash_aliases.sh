...
#            #alias vi="alacritty --title 'NeoVim' --option 'font.size=9.0' --option 'window.dimensions.columns=200' --option 'window.dimensions.lines=60' --command 'nvim -p'"
#        fi
#    else
...
# using subshell (e.g. '(...)') to get rid of the job control messages
# if some output to STDOUT and/or STDERR, pipe those to /dev/null (e.g. '> /dev/null 2>&1 &')

# function to create an Alacritty terminal for production version of my custom LazyNvim
function alacritty-terminal-with-nvim-active {
  (alacritty \
    --title "NeoVim using vi (active on $(hostname))" \
    --option 'font.size=9.0' \
    --option 'window.dimensions.columns=200' \
    --option 'window.dimensions.lines=58' \
    --command nvim "$@" &)
}
alias vi="NVIM_APPNAME=nvim/active alacritty-terminal-with-nvim-active"

# function to create an Alacritty terminal for bata version of my custom LazyNvim
function alacritty-terminal-with-nvim-beta {
  (alacritty \
    --title "NeoVim using vi-beta (beta on $(hostname))" \
    --option 'font.size=9.0' \
    --option 'window.dimensions.columns=200' \
    --option 'window.dimensions.lines=58' \
    --command nvim "$@" &)
}
alias vi-beta="NVIM_APPNAME=nvim/beta alacritty-terminal-with-nvim-beta"

# function to create an Alacritty terminal for experimental version of my custom LazyNvim
function alacritty-terminal-with-nvim-experimental {
  (alacritty \
    --title "NeoVim using vi-exp (experimental on $(hostname))" \
    --option 'font.size=9.0' \
    --option 'window.dimensions.columns=200' \
    --option 'window.dimensions.lines=58' \
    --command nvim "$@" &)
}
alias vi-exp="NVIM_APPNAME=nvim/experimental alacritty-terminal-with-nvim-experimental"

# function to create an Alacritty terminal for untouchec version of LazyNvim
function alacritty-terminal-with-nvim-lazyvim {
  (alacritty \
    --title "NeoVim using vi-lv (lazyvim on $(hostname))" \
    --option 'font.size=9.0' \
    --option 'window.dimensions.columns=200' \
    --option 'window.dimensions.lines=58' \
    --command nvim "$@" &)
...
# function to create an Alacritty terminal for general use
...
# function to create a Alacritty terminal for ROS development
# 'config-file' needed since environment variables can be set via '--option'
function alacritty-terminal-ros {
  (alacritty \
    --config-file ~/.config/alacritty/alacritty-ros.toml \
    "$@" &)
}
