...
def main():
    ...
    parser.add_argument(
        "--password",
        "-p",
        help="The Conductor account password. "
        "This is optional: if omitted, the script will prompt securely for "
        "the password.",
    )
    parser.add_argument(
        "node_addr",
        help="The 36-bit node id, in hex form, or `broadcast` to "
        "send to all nodes with a gateway",
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--payload", help="The payload to send, in hex", type=bytearray.fromhex
    )
    group.add_argument(
        "--counter", action="store_true", help="Continually send a 4-byte counter value"
    )
    parser.add_argument(
        "--gateway", help="Specify the gateway through with the message will be routed."
    )
    parser.add_argument(
        "--ack", help="request an ACK from the node", action="store_true"
    )
    ...
    if args.counter:
        for counter in itertools.count():
            ...
            send_message_checked(
                account, message, args.node_addr, args.gateway, args.ack
            )
    else:
        send_message_checked(
            account, args.payload, args.node_addr, args.gateway, args.ack
        )
